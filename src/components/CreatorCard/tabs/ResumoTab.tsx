...
const ResumoTab = () => {
  ...
  const handleExport = async () => {
    ...
    setIsExporting(true);
    try {
      // ALTERAÇÃO: Mudado para toJpeg e adicionado um fundo sólido para a imagem.
      const dataUrl = await htmlToImage.toJpeg(cardRef.current, { 
        quality: 1, 
        pixelRatio: 2, // Aumenta a resolução para melhor qualidade
        backgroundColor: '#020617' // Cor de fundo (slate-950) para um visual premium
      });
      const link = document.createElement('a');
      // ALTERAÇÃO: Nome do arquivo agora é .jpeg
